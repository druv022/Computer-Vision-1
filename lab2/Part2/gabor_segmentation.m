...
for jj = 1 : length(gaborFilterBank)
    real_out =  imfilter(img_gray, gaborFilterBank(jj).filterPairs(:,:,1), 'symmetric'); % \\TODO: filter the grayscale input with real part of the Gabor
...
for jj = 1:length(featureMaps)
    ...
    featureMags{jj} = (real_part.^2 + imag_part.^2).^(0.5);% \\TODO: Compute the magnitude here
...
if smoothingFlag
    ...
    for jj = 1:length(featureMags)
        filter = fspecial('gaussian',[9,9],5);%rest
%         filter = fspecial('gaussian',[21,21],20);%kobi
%           filter = fspecial('gaussian',[5,5],3);%cows
        smoothed_featureMags = imfilter(featureMags{jj}, filter, 'symmetric');
        features(:,:,jj) = smoothed_featureMags;
    end
    %FOR_LOOP
...
for i = 1:length(features(1,:))
    features(:,i) = (features(:,i)-mean(features(:,i)))/var(features(:,i));
end
% f
